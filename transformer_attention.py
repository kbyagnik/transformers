...
class AttentionQKV(Model):
	...
	def call(self, queries, keys, values, mask=None):
		...
		if len(keys.shape.as_list())==3:
			keysT = tf.transpose(keys, perm=[0, 2, 1])
		else: 
			keysT = tf.transpose(keys, perm=[0, 1, 3, 2])
		#print(queries.shape, keysT.shape)
		similarity = tf.linalg.matmul(queries, keysT) # Compute the similarity according to the QKV formula
		...
		weights = tf.nn.softmax(masked_similarity/tf.math.sqrt(key_dim)) # Turn the similarity into a normalized output
...
class MultiHeadProjection(Model):
	...
	def _split_heads(self, tensor):
		...
		# print(tensor.shape, self.n_heads)
	...
	def _combine_heads(self, tensor):
		...
		# tensorlen = tf.shape(tensor)[1]

		# feature_size = tf.shape(tensor).as_list()[-1]
